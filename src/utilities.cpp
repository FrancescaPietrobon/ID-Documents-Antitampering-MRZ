...
void saveCentersPredictionImage(cv::Mat img, std::vector<myPoint> centers, std::string img_name)
{
    ...
    for(size_t i = 0; i < centers.size(); ++i)
    {
        ...
        color2 = (centers[i].getCluster() + 2) * 20;
        color3 = (centers[i].getCluster() + 2) * 40;

        /*
        if(i != 0 && color[i] != color[i-1]) //FUNZIONA SOLO SE LE LABEL DELLO STESSO TIPO SONO MESSE VICINE
            std::cout << std::endl;
        
        std::cout << dictionary[centers[i].getLabel()];
        */
        //std::cout << color[i] << "  " << dictionary[centers[i].getLabel()] << " \t";
        cv::circle(new_image, cv::Point(centers[i].getX(), centers[i].getY()), 0, cv::Scalar(color1,color2,color3), thickness, lineType);
    }
    std::cout << std::endl;
    cv::imwrite(img_name, new_image);
}
