...
std::string Date::findDate(std::string data)
{
    ...
    std::pair<std::string, size_t> monthWithIndex = this->findMonth(data);
    std::string month = monthWithIndex.first;
    size_t monthIndex = monthWithIndex.second;
    if(month != "None")
        newDate = this->extractAlphanumericDate(data, month, monthIndex);
    else if(this->isNumber(data) && ((data.size() == 6) || (data.size() == 9)))
        newDate = this->extractNumericDate(data);
    return newDate;
}
...
std::pair<std::string, size_t> Date::findMonth(std::string data)
{
    ...
    std::pair<std::string, size_t> monthWithIndex = this->searchMonth(data, monthUCtoNum);
    if(monthWithIndex.first == "None")
        monthWithIndex = this->searchMonth(data, monthLCtoNum);
...
std::string Date::extractAlphanumericDate(std::string data, std::string month, size_t index)
{
    ...
    SPDLOG_INFO("Date {} converted in {}", data, newDate);
    return newDate;
}
...
{
    return (this->isNumber(day) && (std::stoi(day) > 0) && (std::stoi(day) <= 31));
}

bool Date::monthOk(std::string month)
{
    return (this->isNumber(month) && (std::stoi(month) > 0) && (std::stoi(month) <= 12));
}
